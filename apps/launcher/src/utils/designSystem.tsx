...
import {Icon, IconName} from '@app/components/basic/icon';
...
export const getTabBarIcon =
  ...
  ({focused, color, size}: {focused: boolean; color: string; size: number}) =>{
    console.log('%c [ focused, color, size ]-165', 'font-size:13px; background:pink; color:#bf2c9f;',tabName, focused, color, size)

    return <Icon name={getTabIconName(tabName, focused)} size={size} color={color} />;
  }
...
const getTabIconName = (tabName: string, focused: boolean): IconName => {
  ...
  if (tabName === 'MatchTab') {
    return focused ? 'infinite' : 'infinite-outline';
  }
